...
Python's standard library is an interview superpower: `collections`, `heapq`, and `bisect` give
you the right data structure with O(1) or O(log n) operations, so you write less code and it
runs faster. This is a reference chapter — keep it open while you drill the pattern chapters.
...
**Trap:** `xs.pop(0)` and `xs.insert(0, x)` are **O(n)** — they shift every element. For a queue,
use `deque`.
...
Use a `set` for membership/dedup and a `dict` for key→value. Both are O(1) average. `dict`
...
Counter("listen") == Counter("silent")   # True  -> anagram check!
```

Counters add and subtract like multisets (`c1 + c2`, `c1 - c2`) and compare by counts — the
anagram pattern in one line.
...
heapq.heappush(h, 1)
smallest = heapq.heappop(h)    # O(log n) -> 1   (MIN-heap)
...
- The standard library is fair game and expected — reach for it.
