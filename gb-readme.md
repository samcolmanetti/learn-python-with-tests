...
[Learn Go with Tests](https://github.com/quii/learn-go-with-tests). It keeps the same
incremental, test-first rhythm, but aims squarely at interview preparation in Python.
...
4. **Complexity & Python's toolbox**: Big-O, and the built-ins (`deque`, `heapq`, `Counter`,
   `bisect`, and friends) that make Python interview code short and fast.
...
Every chapter links to its runnable code. Clone the repo, run `uv run pytest`, and change
