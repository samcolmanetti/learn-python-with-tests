...
**[You can find all the code for this chapter here](https://github.com/samcolmanetti/learn-python-with-tests/tree/main/code/property_based_testing)**
...
We'll build a Roman-numeral converter and guard it with a property. Make sure Hypothesis is installed (`uv add --dev hypothesis` if you haven't already), then run
...
composites. We also assert that out-of-range numbers raise. Put this in `test_roman.py`:
...
few example tests in `test_roman.py` so the function has something concrete to satisfy, and
...
Add a stubbed `from_roman` to `roman.py`, alongside the `to_roman` you already wrote, that
