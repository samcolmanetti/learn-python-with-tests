...
TEMPLATE_FILES = sorted(CODE_ROOT.glob("*/_template.py"))
...
def test_binary_search_template():
    m = _load(CODE_ROOT / "binary_search/_template.py")
...
def test_two_pointers_template():
    m = _load(CODE_ROOT / "two_pointers/_template.py")
    data = [0, 1, 0, 3, 0, 2]
...
def test_sliding_window_template():
    m = _load(CODE_ROOT / "sliding_window/_template.py")
...
def test_prefix_sum_template():
    m = _load(CODE_ROOT / "prefix_sum/_template.py")
...
def test_mono_stack_template():
    m = _load(CODE_ROOT / "stack_and_monotonic_stack/_template.py")
    assert m.next_greater([2, 1, 2, 4, 3]) == [4, 2, 4, -1, -1]


def test_trees_dfs_template():
    m = _load(CODE_ROOT / "trees_dfs/_template.py")
...
def test_trees_bfs_template():
    dfs = _load(CODE_ROOT / "trees_dfs/_template.py")
    bfs = _load(CODE_ROOT / "trees_bfs/_template.py")
...
def test_graphs_traversal_template():
    m = _load(CODE_ROOT / "graphs_traversal/_template.py")
...
def test_topological_sort_template():
    m = _load(CODE_ROOT / "topological_sort/_template.py")
...
def test_union_find_template():
    m = _load(CODE_ROOT / "union_find/_template.py")
    uf = m.UnionFind()
...
def test_backtracking_template():
    m = _load(CODE_ROOT / "backtracking/_template.py")
...
def test_trie_template():
    m = _load(CODE_ROOT / "trie/_template.py")
    trie = m.Trie()
    trie.insert("cat")
