...
**[You can find all the code for this chapter here](https://github.com/samcolmanetti/learn-python-with-tests/tree/main/lists_and_slicing)**
...
Start small: sum a list of numbers. `lists_and_slicing/v1/test_lists_and_slicing.py`:
...
Loop over the numbers and add them up. `lists_and_slicing/v1/lists_and_slicing.py`:
...
Our next requirement is reversing a list and dropping its head. The `v2` tests:
...
name for the *same* object. A full slice `items[:]` makes an independent copy. The `v3` tests pin
down both halves:
...
copy (`lists_and_slicing/v3/lists_and_slicing.py`):
